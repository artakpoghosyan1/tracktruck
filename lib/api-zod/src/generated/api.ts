/**
 * Generated by orval v8.10.0 🍺
 * Do not edit manually.
 * Api
 * TrackTruck Live API specification
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary Register a new user
 */
export const authSignupBodyPasswordMin = 8;



export const AuthSignupBody = zod.object({
  "email": zod.string().email(),
  "password": zod.string().min(authSignupBodyPasswordMin),
  "name": zod.string()
})


/**
 * @summary Login with email and password
 */
export const AuthLoginBody = zod.object({
  "email": zod.string().email(),
  "password": zod.string()
})

export const AuthLoginResponse = zod.object({
  "accessToken": zod.string(),
  "refreshToken": zod.string(),
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string(),
  "emailVerified": zod.boolean(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "orgRemainingRoutes": zod.number().nullish()
})
})


/**
 * @summary Login or register with Google
 */
export const AuthGoogleBody = zod.object({
  "idToken": zod.string()
})

export const AuthGoogleResponse = zod.object({
  "accessToken": zod.string(),
  "refreshToken": zod.string(),
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string(),
  "emailVerified": zod.boolean(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "orgRemainingRoutes": zod.number().nullish()
})
})


/**
 * @summary Refresh access token
 */
export const AuthRefreshBody = zod.object({
  "refreshToken": zod.string()
})

export const AuthRefreshResponse = zod.object({
  "accessToken": zod.string(),
  "refreshToken": zod.string(),
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string(),
  "emailVerified": zod.boolean(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "orgRemainingRoutes": zod.number().nullish()
})
})


/**
 * @summary Logout
 */
export const AuthLogoutResponse = zod.object({
  "message": zod.string()
})


/**
 * @summary Get current user
 */
export const AuthMeResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string(),
  "emailVerified": zod.boolean(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "orgRemainingRoutes": zod.number().nullish()
})


/**
 * @summary Request a password reset email (not yet implemented)
 */
export const AuthForgotPasswordBody = zod.object({
  "email": zod.string().email()
})


/**
 * @summary Verify email address with a token (not yet implemented)
 */
export const AuthVerifyEmailBody = zod.object({
  "token": zod.string()
})


/**
 * @summary List routes with pagination
 */
export const listRoutesQueryPageDefault = 1;
export const listRoutesQueryPageSizeDefault = 10;
export const listRoutesQuerySortDefault = `newest`;

export const ListRoutesQueryParams = zod.object({
  "page": zod.coerce.number().default(listRoutesQueryPageDefault),
  "page_size": zod.coerce.number().default(listRoutesQueryPageSizeDefault),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']).optional(),
  "search": zod.coerce.string().optional(),
  "sort": zod.enum(['newest', 'oldest']).default(listRoutesQuerySortDefault)
})

export const ListRoutesResponse = zod.object({
  "data": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startLat": zod.number(),
  "startLng": zod.number(),
  "endLat": zod.number(),
  "endLng": zod.number(),
  "truckSpeedMph": zod.number(),
  "shareToken": zod.string().nullish(),
  "shareLinkActive": zod.boolean(),
  "createdAt": zod.date(),
  "updatedAt": zod.date(),
  "updateCount": zod.number().nullish(),
  "etaTargetUtc": zod.date().nullish(),
  "etaTimezone": zod.string().nullish()
})),
  "total": zod.number(),
  "page": zod.number(),
  "pageSize": zod.number()
})


/**
 * @summary Create a new route
 */
export const createRouteBodyTruckSpeedMphDefault = 60;
export const createRouteBodyPolylineItemMin = 2;
export const createRouteBodyPolylineItemMax = 2;



export const CreateRouteBody = zod.object({
  "name": zod.string(),
  "startLat": zod.number(),
  "startLng": zod.number(),
  "endLat": zod.number(),
  "endLng": zod.number(),
  "truckSpeedMph": zod.number().default(createRouteBodyTruckSpeedMphDefault),
  "polyline": zod.array(zod.array(zod.number()).min(createRouteBodyPolylineItemMin).max(createRouteBodyPolylineItemMax)).optional(),
  "speedProfile": zod.array(zod.object({
  "distanceM": zod.number(),
  "speedMph": zod.number()
})).optional(),
  "waypoints": zod.array(zod.object({
  "lat": zod.number(),
  "lng": zod.number(),
  "label": zod.string()
})).optional()
})


/**
 * @summary Get route details
 */
export const GetRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const getRouteResponsePolylineItemMin = 2;
export const getRouteResponsePolylineItemMax = 2;

export const getRouteResponseStopsItemStopTypeDefault = `stop`;

export const GetRouteResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startLat": zod.number(),
  "startLng": zod.number(),
  "endLat": zod.number(),
  "endLng": zod.number(),
  "truckSpeedMph": zod.number(),
  "polyline": zod.array(zod.array(zod.number()).min(getRouteResponsePolylineItemMin).max(getRouteResponsePolylineItemMax)),
  "speedProfile": zod.array(zod.object({
  "distanceM": zod.number(),
  "speedMph": zod.number()
})).optional(),
  "distanceM": zod.number(),
  "estimatedDurationS": zod.number(),
  "etaTargetUtc": zod.date().nullish(),
  "etaTimezone": zod.string().nullish(),
  "shareToken": zod.string().nullish(),
  "shareLinkActive": zod.boolean(),
  "stops": zod.array(zod.object({
  "id": zod.number(),
  "routeId": zod.number(),
  "name": zod.string(),
  "lat": zod.number(),
  "lng": zod.number(),
  "durationMinutes": zod.number(),
  "sortOrder": zod.number(),
  "stopType": zod.enum(['stop', 'pacing']).default(getRouteResponseStopsItemStopTypeDefault),
  "createdAt": zod.date()
})),
  "waypoints": zod.array(zod.object({
  "lat": zod.number(),
  "lng": zod.number(),
  "label": zod.string()
})),
  "createdAt": zod.date(),
  "updatedAt": zod.date(),
  "updateCount": zod.number().nullish()
})


/**
 * @summary Update a route
 */
export const UpdateRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const updateRouteBodyPolylineItemMin = 2;
export const updateRouteBodyPolylineItemMax = 2;



export const UpdateRouteBody = zod.object({
  "name": zod.string().optional(),
  "startLat": zod.number().optional(),
  "startLng": zod.number().optional(),
  "endLat": zod.number().optional(),
  "endLng": zod.number().optional(),
  "truckSpeedMph": zod.number().optional(),
  "polyline": zod.array(zod.array(zod.number()).min(updateRouteBodyPolylineItemMin).max(updateRouteBodyPolylineItemMax)).optional(),
  "speedProfile": zod.array(zod.object({
  "distanceM": zod.number(),
  "speedMph": zod.number()
})).optional(),
  "waypoints": zod.array(zod.object({
  "lat": zod.number(),
  "lng": zod.number(),
  "label": zod.string()
})).optional()
})

export const updateRouteResponsePolylineItemMin = 2;
export const updateRouteResponsePolylineItemMax = 2;

export const updateRouteResponseStopsItemStopTypeDefault = `stop`;

export const UpdateRouteResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startLat": zod.number(),
  "startLng": zod.number(),
  "endLat": zod.number(),
  "endLng": zod.number(),
  "truckSpeedMph": zod.number(),
  "polyline": zod.array(zod.array(zod.number()).min(updateRouteResponsePolylineItemMin).max(updateRouteResponsePolylineItemMax)),
  "speedProfile": zod.array(zod.object({
  "distanceM": zod.number(),
  "speedMph": zod.number()
})).optional(),
  "distanceM": zod.number(),
  "estimatedDurationS": zod.number(),
  "etaTargetUtc": zod.date().nullish(),
  "etaTimezone": zod.string().nullish(),
  "shareToken": zod.string().nullish(),
  "shareLinkActive": zod.boolean(),
  "stops": zod.array(zod.object({
  "id": zod.number(),
  "routeId": zod.number(),
  "name": zod.string(),
  "lat": zod.number(),
  "lng": zod.number(),
  "durationMinutes": zod.number(),
  "sortOrder": zod.number(),
  "stopType": zod.enum(['stop', 'pacing']).default(updateRouteResponseStopsItemStopTypeDefault),
  "createdAt": zod.date()
})),
  "waypoints": zod.array(zod.object({
  "lat": zod.number(),
  "lng": zod.number(),
  "label": zod.string()
})),
  "createdAt": zod.date(),
  "updatedAt": zod.date(),
  "updateCount": zod.number().nullish()
})


/**
 * @summary Delete a route
 */
export const DeleteRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const DeleteRouteResponse = zod.object({
  "message": zod.string()
})


/**
 * @summary Add a stop to a route
 */
export const CreateStopParams = zod.object({
  "id": zod.coerce.number()
})

export const createStopBodyDurationMinutesDefault = 5;
export const createStopBodyStopTypeDefault = `stop`;

export const CreateStopBody = zod.object({
  "name": zod.string(),
  "lat": zod.number(),
  "lng": zod.number(),
  "durationMinutes": zod.number().default(createStopBodyDurationMinutesDefault),
  "sortOrder": zod.number().optional(),
  "stopType": zod.enum(['stop', 'pacing']).default(createStopBodyStopTypeDefault)
})


/**
 * @summary Update a stop
 */
export const UpdateStopParams = zod.object({
  "id": zod.coerce.number(),
  "stopId": zod.coerce.number()
})

export const UpdateStopBody = zod.object({
  "name": zod.string().optional(),
  "lat": zod.number().optional(),
  "lng": zod.number().optional(),
  "durationMinutes": zod.number().optional(),
  "sortOrder": zod.number().optional()
})

export const updateStopResponseStopTypeDefault = `stop`;

export const UpdateStopResponse = zod.object({
  "id": zod.number(),
  "routeId": zod.number(),
  "name": zod.string(),
  "lat": zod.number(),
  "lng": zod.number(),
  "durationMinutes": zod.number(),
  "sortOrder": zod.number(),
  "stopType": zod.enum(['stop', 'pacing']).default(updateStopResponseStopTypeDefault),
  "createdAt": zod.date()
})


/**
 * @summary Delete a stop
 */
export const DeleteStopParams = zod.object({
  "id": zod.coerce.number(),
  "stopId": zod.coerce.number()
})

export const DeleteStopResponse = zod.object({
  "message": zod.string()
})


/**
 * @summary Activate a route (requires payment)
 */
export const ActivateRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const ActivateRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.string(),
  "shareToken": zod.string()
})


/**
 * @summary Start route simulation
 */
export const StartRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const StartRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startedAt": zod.date().nullish(),
  "pausedAt": zod.date().nullish(),
  "effectiveElapsedMs": zod.number(),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish()
})


/**
 * @summary Pause route simulation
 */
export const PauseRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const PauseRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startedAt": zod.date().nullish(),
  "pausedAt": zod.date().nullish(),
  "effectiveElapsedMs": zod.number(),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish()
})


/**
 * @summary Resume route simulation
 */
export const ResumeRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const ResumeRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startedAt": zod.date().nullish(),
  "pausedAt": zod.date().nullish(),
  "effectiveElapsedMs": zod.number(),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish()
})


/**
 * @summary Reset route simulation
 */
export const ResetRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const ResetRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startedAt": zod.date().nullish(),
  "pausedAt": zod.date().nullish(),
  "effectiveElapsedMs": zod.number(),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish()
})


/**
 * @summary Recalculate route after revision
 */
export const RecalculateRouteParams = zod.object({
  "id": zod.coerce.number()
})

export const RecalculateRouteResponse = zod.object({
  "routeId": zod.number(),
  "status": zod.enum(['draft', 'ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startedAt": zod.date().nullish(),
  "pausedAt": zod.date().nullish(),
  "effectiveElapsedMs": zod.number(),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish()
})


/**
 * @summary Get public route data by share token
 */
export const GetPublicTrackParams = zod.object({
  "token": zod.coerce.string()
})

export const getPublicTrackResponsePolylineItemMin = 2;
export const getPublicTrackResponsePolylineItemMax = 2;



export const GetPublicTrackResponse = zod.object({
  "routeId": zod.number(),
  "routeName": zod.string(),
  "status": zod.enum(['ready', 'in_progress', 'paused', 'completed', 'expired']),
  "startLat": zod.number(),
  "startLng": zod.number(),
  "endLat": zod.number(),
  "endLng": zod.number(),
  "polyline": zod.array(zod.array(zod.number()).min(getPublicTrackResponsePolylineItemMin).max(getPublicTrackResponsePolylineItemMax)),
  "stops": zod.array(zod.object({
  "name": zod.string(),
  "lat": zod.number(),
  "lng": zod.number(),
  "durationMinutes": zod.number(),
  "sortOrder": zod.number()
})),
  "distanceM": zod.number(),
  "estimatedDurationS": zod.number(),
  "snapshot": zod.object({
  "routeId": zod.number(),
  "timestamp": zod.date(),
  "status": zod.enum(['ready', 'in_progress', 'paused', 'completed']),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish(),
  "speedMph": zod.number().optional(),
  "atStopName": zod.string().nullish(),
  "etaTargetUtc": zod.date().nullish(),
  "etaTimezone": zod.string().nullish()
}).optional()
})


/**
 * Upgrade to WebSocket connection for real-time truck position updates.
Client connects via ws(s)://<host>/api/public/ws/track/{token}.
Server pushes LiveSnapshotResponse JSON messages at regular intervals.
Connection closes when route completes or token expires.

 * @summary WebSocket endpoint for live truck tracking
 */
export const WsPublicTrackParams = zod.object({
  "token": zod.coerce.string()
})


/**
 * @summary Get current simulation state
 */
export const GetPublicTrackStateParams = zod.object({
  "token": zod.coerce.string()
})

export const GetPublicTrackStateResponse = zod.object({
  "routeId": zod.number(),
  "timestamp": zod.date(),
  "status": zod.enum(['ready', 'in_progress', 'paused', 'completed']),
  "distanceTraveledM": zod.number(),
  "progressPercent": zod.number(),
  "lat": zod.number().nullish(),
  "lng": zod.number().nullish(),
  "bearing": zod.number().nullish(),
  "speedMph": zod.number().optional(),
  "atStopName": zod.string().nullish(),
  "etaTargetUtc": zod.date().nullish(),
  "etaTimezone": zod.string().nullish()
})


/**
 * @summary Get IANA timezone for geographic coordinates
 */
export const GetTimezoneQueryParams = zod.object({
  "lat": zod.coerce.number(),
  "lng": zod.coerce.number()
})

export const GetTimezoneResponse = zod.object({
  "timezone": zod.string().describe('IANA timezone identifier (e.g. \"America\/Phoenix\")'),
  "label": zod.string().describe('Human-readable timezone label (e.g. \"Mountain Standard Time\")')
})


/**
 * @summary List all allowed emails and their roles
 */
export const ListAllowedEmailsResponseItem = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string().nullish(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date()
})
export const ListAllowedEmailsResponse = zod.array(ListAllowedEmailsResponseItem)


/**
 * @summary Add a new email to the allowed list
 */
export const addAllowedEmailBodyRoleDefault = `user`;
export const addAllowedEmailBodyIsPaidDefault = true;
export const addAllowedEmailBodyRouteLimitDefault = 25;

export const AddAllowedEmailBody = zod.object({
  "email": zod.string().email(),
  "name": zod.string().optional(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']).default(addAllowedEmailBodyRoleDefault),
  "isPaid": zod.boolean().default(addAllowedEmailBodyIsPaidDefault),
  "routeLimit": zod.number().default(addAllowedEmailBodyRouteLimitDefault),
  "organizationId": zod.number().optional()
})


/**
 * @summary Remove an email from the allowed list
 */
export const RemoveAllowedEmailParams = zod.object({
  "email": zod.coerce.string().email()
})

export const RemoveAllowedEmailResponse = zod.object({
  "message": zod.string()
})


/**
 * @summary Update settings for an allowed email
 */
export const UpdateAllowedEmailParams = zod.object({
  "email": zod.coerce.string().email()
})

export const UpdateAllowedEmailBody = zod.object({
  "name": zod.string().optional(),
  "isPaid": zod.boolean().optional(),
  "routeLimit": zod.number().optional(),
  "usedRoutes": zod.number().optional(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']).optional(),
  "organizationId": zod.number().nullish()
})

export const UpdateAllowedEmailResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string().nullish(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date()
})


/**
 * @summary List all organizations
 */
export const ListOrganizationsResponseItem = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "allocatedRoutes": zod.number(),
  "usedRoutes": zod.number(),
  "createdAt": zod.date()
})
export const ListOrganizationsResponse = zod.array(ListOrganizationsResponseItem)


/**
 * @summary Create a new organization
 */
export const createOrganizationBodyIsPaidDefault = false;
export const createOrganizationBodyRouteLimitDefault = 0;

export const CreateOrganizationBody = zod.object({
  "name": zod.string(),
  "isPaid": zod.boolean().default(createOrganizationBodyIsPaidDefault),
  "routeLimit": zod.number().default(createOrganizationBodyRouteLimitDefault)
})


/**
 * @summary Update an organization
 */
export const UpdateOrganizationParams = zod.object({
  "id": zod.coerce.number()
})

export const UpdateOrganizationBody = zod.object({
  "name": zod.string().optional(),
  "isPaid": zod.boolean().optional(),
  "routeLimit": zod.number().optional()
})

export const UpdateOrganizationResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "allocatedRoutes": zod.number(),
  "usedRoutes": zod.number(),
  "createdAt": zod.date()
})


/**
 * @summary Delete an organization
 */
export const DeleteOrganizationParams = zod.object({
  "id": zod.coerce.number()
})

export const DeleteOrganizationResponse = zod.object({
  "message": zod.string()
})


/**
 * @summary List users in the caller's organization
 */
export const ListOrgUsersResponse = zod.object({
  "organization": zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "allocatedRoutes": zod.number(),
  "usedRoutes": zod.number(),
  "createdAt": zod.date()
}).nullable(),
  "members": zod.array(zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string().nullish(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date()
}))
})


/**
 * @summary Add a user to the organization
 */
export const addOrgUserBodyRouteLimitDefault = 0;

export const AddOrgUserBody = zod.object({
  "email": zod.string().email(),
  "name": zod.string().optional(),
  "routeLimit": zod.number().default(addOrgUserBodyRouteLimitDefault)
})


/**
 * @summary Update a user's quota allocation
 */
export const UpdateOrgUserParams = zod.object({
  "email": zod.coerce.string()
})

export const UpdateOrgUserBody = zod.object({
  "routeLimit": zod.number(),
  "name": zod.string().optional()
})

export const UpdateOrgUserResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "name": zod.string().nullish(),
  "role": zod.enum(['super_admin', 'admin', 'org_admin', 'user']),
  "isPaid": zod.boolean(),
  "routeLimit": zod.number(),
  "usedRoutes": zod.number(),
  "organizationId": zod.number().nullish(),
  "createdAt": zod.date()
})


/**
 * @summary Remove a user from the organization
 */
export const RemoveOrgUserParams = zod.object({
  "email": zod.coerce.string()
})

export const RemoveOrgUserResponse = zod.object({
  "message": zod.string()
})


